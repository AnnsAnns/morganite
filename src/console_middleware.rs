...
pub async fn handle_console( state: Arc<Mutex<Shared>>) -> Result<(), Box<dyn Error>>{
    ...
    let cmd_receiver = state.lock().await.console_cmd_receiver.clone();
    ...
    loop {
        ...
        // Still required a Arc<Mutex<>> since the compiler doesn't know that
        let cmd_receiver_arc = cmd_receiver.clone();
        let mut cmd_receiver = cmd_receiver_arc.lock().await;

        tokio::select! {
            //another async task has a message to be send to the user
            Some(event) = rx.recv() => {
                // Send the event to the TUI for visualization
                state.lock().await.console_input_sender.send(event)?;
            }
            Some(event) = cmd_receiver.recv() => {
                // If the channel event is type routing, ignore it
                tracing::info!("Received command: {:?}", event);
            }
